...
CREATE TABLE IF NOT EXISTS `KelenFIla`.`methodPaiement` (
  ...
  INDEX `FK_PAIEMENT_USER_MET_idx` (`idUser` ASC) ,
...
CREATE TABLE IF NOT EXISTS `KelenFIla`.`categorie` (
  ...
  UNIQUE INDEX `designCategorie_UNIQUE` (`designCategorie` ASC) )
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `KelenFIla`.`enchere` (
  ...
  UNIQUE INDEX `numEnchere_UNIQUE` (`numEnchere` ASC) ,
  UNIQUE INDEX `idVendeur_UNIQUE` (`idVendeur` ASC) ,
  INDEX `FK_ENCHERE_CATEGORIE_idx` (`idCategorie` ASC) ,
...
CREATE TABLE IF NOT EXISTS `KelenFIla`.`lot` (
  ...
  `idEnchere` INT NULL,
  PRIMARY KEY (`idLot`),
  UNIQUE INDEX `idEnchere_UNIQUE` (`idEnchere` ASC) ,
...
CREATE TABLE IF NOT EXISTS `KelenFIla`.`participer` (
  ...
  INDEX `fk_user_has_enchere_enchere1_idx` (`idEnchere_enchere` ASC) ,
  INDEX `fk_user_has_enchere_user1_idx` (`idUser_client` ASC) ,
...
CREATE TABLE IF NOT EXISTS `KelenFIla`.`adjudicataire` (
  ...
  INDEX `fk_user_has_lot_lot1_idx` (`idLot_lot` ASC),
  INDEX `fk_user_has_lot_user1_idx` (`idUser_adju` ASC),
