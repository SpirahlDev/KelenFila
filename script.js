...
$(document).ready(function(){
    ...
    $(".exit-btn").click(function(){
        ...
        btnOui.addEventListener("click", () => {
            console.log("Clique sur le bouton oui !");
            socket.send("quit");
        });
          
        btnNon.addEventListener("click", () => {
            console.log("Clique sur le bouton non !");
...
socket.addEventListener('message', function (event) {
    ...
    if(event.data==="enchere"){
        displayMsg(event.data.enchere);
    }
    else if(event.data==="lot"){
        displayLot(event.data.lot);
    }
});
...
function displayMsg(message){  
    ...
    //utiliser cette fonction pour afficher les informations contextuelles
...
function displayLot(lot){
    let img=document.querySelector(".item-img img");
    img.setAttribute("src",lot.image1);
    $("#design-Lot").text(lot.designLot);
    $("#numero-Lot").text(lot.numeroLot);
    $("#etat").text(lot.etatLot);
    // $("#").text(lot.designCategorie);
    $("#text-descript p").text(lot.descriptionLot);
}